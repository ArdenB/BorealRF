...
__title__ = "Random Forest Classification Implementation"
...
def main():
	...
	folder = "./pyEWS/experiments/2.Classification/"
	tmpath = "./pyEWS/experiments/2.Classification/tmp/"
	cf.pymkdir(folder)
	cf.pymkdir(tmpath)
	...
	versions["v1_sols"] = ({
		"desc":"Testing sols two stage RF classification approache",
		"teststr":"_V1_RFCsol", #string to add to written files
		# "windows":np.arange(4, 20),
		# "windows":[3, 4, 5, 10, 15, 20],
		"clfy":"sol", #classification methods
		"testvariable" : "Classification",
		"maxiter":11 # The maximum number to tests to run. determined observationally in v1
	})

	for experiment  in [0, 1, 2, 3, 4, 5, 6, 7]:
		for vers in versions:
			fn = recur_classification(versions, vers, window, tmpath, test_size, experiment, 
				folder, ntree, SelMethod, VarImp, force )
			writenames.append(fn)
...
# ============================ Two stange modeling =============================
# ==============================================================================
def classed_regression(versions, vers, window, tmpath, test_size, experiment, folder, ntree, SelMethod, VarImp, force):
	"""Funtion takes the classified data and then runs it through a regression approach"""
...
def recur_classification(versions, vers, window, tmpath, test_size, experiment, folder, ntree, SelMethod, VarImp, force):
	"""
	Funtion aims to recursvilly test and train the classification 
	"""
	# ========== Set the max itteration ==========
	maxiter = versions[vers]["maxiter"]
	...
	# ========== Make a file name =============
	fnout   =  folder + "S05_RF_TestingCLASS_Exp%02d_%s_classifcation.csv" % (experiment, versions[vers]["teststr"])
	fnsite  =  folder + "S05_RF_TestingCLASS_Exp%02d_%s_classifcation_siteinfo.csv" % (experiment, versions[vers]["teststr"])
	fnthres = folder + "S05_RF_TestingCLASS_Exp%02d_%s_classifcation_thresh.csv" % (experiment, versions[vers]["teststr"])
	...
	for branch in test_branchs:
		...
		X_train, X_test, y_train, y_test, col_nms, loadstats, corr, df_site, thrsh = df_proccessing(
			window, tmpath, test_size, branch, experiment, cols_keep=ColNm, recur=None, 
			final = branch == (maxiter-1), classify= versions[vers]["clfy"], thrsh=thrsh)

		# # ========== Calculate the random forest ==========
		time,  r2, feature_imp  = skl_rf_regression(
			X_train, X_test, y_train, y_test, col_nms, ntree, test_size, 
			branch, window, tmpath, maxiter, experiment, vers, verbose=True, perm=True, regress=False)
	...
	perf_df.to_csv(fnout)
	if experiment == 0:
		df_site.to_csv(fnsite)
		pd.DataFrame(thrsh).reset_index().rename({"index":"percentile"}, axis=1).to_csv(fnthres)
	# writenames.append(fnout)
	return fnout
	# breakpoint()
...
def Summaryplots(writenames):
	...
	exp_df["experiment"] = exp_df["experiment"].astype('category')
	...
	for va in ["TimeCumulative", "R2", "NumVar",'SiteFraction', "R2perVar"]:
		...
		# +++++ make some plots +++++
		plt.figure(va)
		ax = sns.lineplot(x="BranchName", y=va, data=exp_df,  err_style='bars', hue="experiment")
	...
	ax = sns.lineplot(x="NumVar", y="R2", data=exp_df, err_style='band', hue="experiment")
	for label in ax.get_xticklabels():label.set_rotation(90)
	plt.show()

	outofaccurach = []
	for experiment in range(8):
		path = "./pyEWS/experiments/2.Classification/"
		fnameout = path + "S05_twostage_10yrs_10_exp%02d_v1_sols.csv" % (experiment)
		ds_in = pd.read_csv(fnameout).rename(columns={'Unnamed: 0':'BranchName'})
		ds_in["experiment"] = experiment
		outofaccurach.append(ds_in)
	ds_ac = pd.concat(outofaccurach)
	ds_ac["experiment"] = ds_ac["experiment"].astype('category')
	sns.violinplot(x="Observed", y="Estimated", hue="experiment", data=ds_ac)
...
def skl_rf_regression( X_train, X_test, y_train, y_test, col_nms, ntree, 
	test_size, branch, test, tmpath, maxiter, experiment, vers, cores=-1, verbose=False, perm=False, regress=True):
	"""
	...
	if regress:
		...
		# regressor.fit(X_train, y_train.astype("float32").values)
	...
	if branch == (maxiter - 1):
		...
		_predictedVSobserved(y_test, y_pred, branch, test, tmpath, experiment, vers)

	return tDif, sklMet.r2_score(y_test, y_pred), FI


def _predictedVSobserved(y_test, y_pred, branch, test, tmpath, experiment, vers):
	"""
	...
	fnameout = path + "S05_twostage_%02dyrs_%02d_exp%02d_%s.csv" % (test, branch, experiment, vers)
	print(fnameout)
	dfy.to_csv(fnameout)
# ==============================================================================
# ============================== Data processing ===============================
...
def df_proccessing(window, tmpath, test_size, branch, experiment,  dftype="pandas", 
	cols_keep=None, recur=None, verbose = True, final=False, classify =None, thrsh=None, df_site=None):
	"""
	...
	if df_site is None:
		...
		# ========== Take the spatial infomation and join it to the sites ==========
		df_join = pd.merge(	vi_df.reset_index(), site_df, 	how='left', left_on = 'site', right_on = 'Plot_ID').set_index("index")
		# ========== Pull out the relevant columns ==========
		df_site =  df_join[["site", "Longitude", "Latitude", "Region", "lagged_biomass"]]
		site_df = None
	else:
		df_site = None
	...
	if classify == "sol":
		...
		if thrsh is None: 
			thrsh = vi_df.lagged_biomass.quantile(np.linspace(0,1,8))
	...
	def _quicksave(dftype, tmpath, df, dfname, branch, experiment, window):
		...
		# +++++ make a file name ++++++ 
		fnout = tmpath + "S05tmp_final_%s_%03d_win%02d_exp%02s" % (dfname, branch,  window, experiment)
		if not classify is None:
			fnout += "_classify_%s" % classify
		fnout +=".csv"
	...
	return X_train, X_test, y_train, y_test, cols_out, statsOD, corr, df_site, thrsh
